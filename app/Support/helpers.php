        ...
        for ($i = 0; $i < count($records['data']); $i++) {
            foreach ($buttons as $key => $button) {
                ...
                if (!empty($css_class) && !empty($uri) && !empty($text)) {
                    ...
                    $records['data'][$i]['details'] .= "<a class='btn btn-default " . $css_class . "' href='" . $uri . "'>" . $text . "</a>";
                    ...
                    if (request()->is('follow_contracts/*/*')) {
                        // -- Dual state status
                        if ($records['data'][$i]['status'] == 'pending') {
                            // If it has contract employee (Ishaar)
                            if (count($records['data'][$i]['employees']) > 0) {
                                if (in_array($key,
                                    Constants::CONTRACT_STATUSES_MAP[$records['data'][$i]['status'] . '_acc'][request()->route()->parameter('prvd_benf')])) {
//                                    foreach (Constants::CONTRACT_STATUSES_MAP[$records['data'][$i]['status'] . '_acc'][request()->route()->parameter('prvd_benf')] as $k => $v) {
...
//                                    foreach (Constants::CONTRACT_STATUSES_MAP[$records['data'][$i]['status']][request()->route()->parameter('prvd_benf')] as $k => $v) {
                                    $records['data'][$i]['follow_contract_options'] .= "<a class='btn btn-default " . $buttons[$key]['css_class'] . "' href='" . datatableBtnURI($buttons[$key],
                                            $records['data'][$i]) . "'>" . $buttons[$key]['text'] . "</a>";
//                                    }
                                }
                            }
                        } elseif ($records['data'][$i]['status'] == 'approved') {
