...
@section('title', trans('governments_registeration.headings.list'))
@section('content')
        <!-- BEGIN BREADCRUMBS -->
<div class="breadcrumbs">
    <h1>{{ trans('governments_registeration.headings.list') }}</h1>
    <ol class="breadcrumb">
        <li>
            <a href="{{ url('admin') }}">{{trans('user.home')}}</a>
        </li>
        <li class="active">{{ trans('governments_registeration.headings.list') }}</li>
    </ol>
</div>
<!-- END BREADCRUMBS -->
<div class="m-heading-1 border-green m-bordered">
    <h3> {{ trans('governments_registeration.headings.list') }} </h3>
    <p> {{ trans('governments_registeration.sub-headings.list') }} </p>
</div>
<div class="row">
    <div class="col-md-12">
        <div class="portlet box green">
            <div class="portlet-title">
                <div class="caption">
                    ...
                    <span class="caption-subject bold uppercase"> {{ trans('governments_registeration.headings.list') }}</span>
                </div>
            </div>
                        ...
                        <div class="col-md-6 pull-right">
                            <div class="btn-group pull-right">
                                <button data-toggle="modal" data-target="#main"
                                        data-href="{{ url('admin/users/governments_registeration/create') }}"
                                ...
                                <tr class="odd gradeX {{ $single->hashids }}">
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-lg btn-group-solid margin-bottom-10">
                                            <button data-toggle="modal" data-target="#main"
                                                    data-href="{{ route('admin.users.governments_registeration.edit', $single->hashids) }}"
                                                    class="btn blue">
                                                <i class="fa fa-edit"></i> {{ trans('labels.edit') }}
                                            </button>
                                            <button data-popout="true" data-token="{{ csrf_token() }}"
                                                    data-id="{{ $single->hashids }}"
                                                    data-hreff="{{ route('admin.users.governments_registeration.destroy', $single->hashids) }}"
