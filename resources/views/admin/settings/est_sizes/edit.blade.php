<div class="modal-header">
   ...
   <h4 class="modal-title"><i class="fa fa-edit"></i> {{ trans('est_sizes.headings') }}</h4>
</div>
@if(isset($est_size))
{{ Form::model($est_size, ["files"=>"true", 'route' => ['admin.settings.estsizes.update', $est_size->hashids], 'method' => 'patch', 'id'=>'form']) }}
@else
{{ Form::open(['route' => 'admin.settings.estsizes.store', "files"=>"true", 'id'=>'form_est_sizes']) }}
@endif
      ...
      <div class="col-sm-12">
         <div class="form-body">
            <div class="form-group form-md-line-input form-md-floating-label">
               ...
               <label {{ isset($est_size) ? 'style=top:0;' : "" }} for="name">{{ trans('est_sizes.attributes.ar-name') }} <span class="required">*</span></label>
               <span class="help-block">{{ trans('est_sizes.attributes.ar-name') }}</span>
            </div>
         </div>
      </div>
   </div>
